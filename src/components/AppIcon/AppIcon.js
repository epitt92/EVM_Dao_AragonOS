...
import iconSvgTokens from './assets/app-tokens.svg'
import iconSvgVoting from './assets/app-voting.svg'
import iconSvgFinance from './assets/app-finance.svg'
...
const KNOWN_ICONS = new Map([
  ...
  ['Tokens', iconSvgTokens],
  ['Voting', iconSvgVoting],
  ['Finance', iconSvgFinance]
])
...
const AppIconContent = React.memo(({ app, size, src }) => {
  ...
  if (app && KNOWN_ICONS.has(app.appId)) {
    console.log("appid", app.appId, KNOWN_ICONS.get(app.appId))
    return <IconBase size={size} src={KNOWN_ICONS.get(app.appId)} />
  }

  if (app && KNOWN_ICONS.has(app.name)) {
    return <IconBase size={size} src={KNOWN_ICONS.get(app.name)} />
  }
